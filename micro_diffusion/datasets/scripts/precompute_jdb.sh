...
# C. Precompute latents across multiple GPUs. --multi_gpu --num_processes $num_gpus
python -c "from streaming.base.util import clean_stale_shared_memory; clean_stale_shared_memory()"
for split in train valid; do
    accelerate launch micro_diffusion/datasets/prepare/jdb/precompute.py --datadir "${datadir}/mds/$split/" \
