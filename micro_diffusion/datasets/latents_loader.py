...
class StreamingLatentsDataset (StreamingDataset):
    ...
    def __getitem__ (self, index:int)-> Dict[str, Union[torch.Tensor, str, float]]:
        # get item of StreamingDataset parent class
        sample = super().__getitem__(index)
        out = {}

        # Mas for zero'ed out captions in classifier-free guidance (cfg) training.
        ...
        out['caption_latents'] = torch.from_numpy(
            np.frombuffer(sample['caption_latents'], dtype=np.float16).copy()
        ).reshape(1, self.cap_seq_size, self.cap_emb_dim) # (1, 77, 1024)

        # load either 256 or 512 vae latents depending on current training phase (256 or 512 pretrain/finetune)
        if self.image_size == 256 and 'latents_256' in sample:
            out['image_latents'] = torch.from_numpy(
                np.frombuffer(sample['latents_256'], dtype=np.float16).copy()
            ).reshape(-1, 32, 32) # (4, 32, 32)

        if self.image_size == 512 and 'latents_512' in sample:
            out['image_latents'] = torch.from_numpy(
                np.frombuffer(sample['latents_512'], dtype=np.float16).copy()
            ).reshape(-1, 64, 64) # (4, 64, 64)
